    ...
    def create_stdio_server(self) -> Any:
        ...
        @mcp.tool(
            ...
            description=(
                ...
                "You can embed file snippets and diffs in messages. Paths MUST be "
                "absolute (e.g. /Users/you/repo/src/app.py) — relative paths are "
                "rejected and will be rendered as a '[Path must be absolute: …]' "
                "placeholder in the user's browser.\n"
                "- [[file]](/abs/path/to/file) — embed a text file with syntax highlighting,\n"
                "  or an image (png/jpg/jpeg/gif/webp/avif/bmp/svg) inline\n"
                "- [[file 8:18]](/abs/path/to/file) — embed lines 8-18\n"
                "- [[diff]](/abs/path/to/file) — embed that file's git diff\n"
                "- [[diff]](/abs/path/a|/abs/path/b) — diff two files against each other\n"
                "Embeds render with syntax highlighting in the browser; images open "
                "in a zoomable lightbox on click."
            ),
        )
        async def send(
            message: str,
